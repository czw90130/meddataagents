...
class EuclideanDistanceExampleSelector:
    ...
    def __init__(self, train_json_path=None, train_embeddings_path=None) -> None:
        if train_json_path is None:
            train_json_path = os.path.join(os.path.dirname(__file__), "nl2sql_train.json")
        ...
        if train_embeddings_path is None:
            train_embeddings_path = "./.cache/train_embeddings.npy"
        
        try:
            self.train_embeddings = np.load(train_embeddings_path)
        except FileNotFoundError:
            print("调试：创建嵌入词表")
            os.makedirs(os.path.dirname(train_embeddings_path), exist_ok=True)
            self.train_embeddings = self.bert_model.encode(
                self.train_questions,
            )
            np.save(train_embeddings_path, self.train_embeddings)
...
class DailSQLPromptGenerator:
    ...
    def __init__(
        ...
        try:
            ...
            print(f"错误: 初始化 DailSQLPromptGenerator 时发生异常: {str(e)}")
            print("错误堆栈跟踪:")
            traceback.print_exc()
