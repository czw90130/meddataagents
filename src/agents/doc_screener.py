    ...
    def __init__(self):
        self.agent = DictDialogAgent(
            ...
            sys_prompt=(
                ...
                "3. When evaluating SQL import suitability, consider the following:\n"
                ...
                "4. For table structures, carefully examine if line breaks within cells are represented as '\\n'. "
                "If so, this does not necessarily indicate a broken table structure and may still be suitable for direct SQL import.\n\n"
                "5. When determining table types, consider that the first column might not always be a row header. "
        ...
        self.parser = MarkdownJsonDictParser(
            content_hint={
                ...
                "sql_import": "Assessment of suitability for SQL import (NULL, NO, TRANS, or YES). Consider that long text with line breaks ('\\n') in cells does not indicate a broken table structure.",
    ...
    def __call__(self, input_data):
        ...
        if isinstance(input_data, str):
            if os.path.isfile(input_data):
                # 输入是文件路径
                content, files, md_path = file2text(input_data)
...
if __name__ == "__main__":
    ...
    print(f"SQL导入适用性: {result.metadata['sql_import']}")
